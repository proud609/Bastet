def import_workflow(workflow_path: str, n8n_api_url: str):
  ...
  imported_workflows = []
  existing_workflows = requests.get(f"{n8n_api_url}/workflows", headers=HEADERS).json()["data"]
  existing_workflow_names = [workflow["name"] for workflow in existing_workflows]
  main_workflow_file_name = "main_workflow.json"

  # Scan workflow files
  workflow_files = [f for f in os.listdir(workflow_path) if f.endswith(".json")]
  workflow_files = [f for f in workflow_files if f != main_workflow_file_name]
  ...
  main_workflow_path = os.path.join(workflow_path, main_workflow_file_name)
  with open(main_workflow_path, "r") as file:
    ...
    if main_workflow_name in existing_workflow_names:
      ...
      try:
        response = requests.post(f"{n8n_api_url}/workflows", headers=HEADERS, json=filtered_main_workflow_data)
  ...
  for filename in selected_files:
    if filename.endswith(".json"):
      ...
      with open(file_path, "r") as file:
        ...
        try:
          response = requests.post(f"{n8n_api_url}/workflows", headers=HEADERS, json=filtered_workflow_data)
  ...
  try:
    tags = requests.get(f"{n8n_api_url}/tags", headers=HEADERS).json()
    processor_tag = None
    if any(tag["name"] == "processor" for tag in tags['data']):
      ...
      tqdm.write("\033[94mℹ️  Tag 'processor' already exists.\033[0m")
        ...
        processor_tag = requests.post(f"{n8n_api_url}/tags", headers=HEADERS, json={"name": "processor"}).json()
  ...
  for workflow in imported_workflows:
    ...
    try:
      response = requests.put(f"{n8n_api_url}/workflows/{workflow_id}/tags", headers=HEADERS, json=updated_tags)
